...
public class SwiftHealthPlugin: NSObject, FlutterPlugin {
    ...
    func getData(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        let query = HKSampleQuery(
            ...
            case let (samplesWorkout as [HKWorkout]) as Any:
                ...
                if #available(iOS 13.0, *) {
                    Task {
                        let dictionaries = await self.serializeWorkouts(samplesWorkout)
                        DispatchQueue.main.async {
                            result(dictionaries)
                        }
                    }
                } else {
                    let dictionaries = self.serializeWorkoutsFallback(samplesWorkout)
                    DispatchQueue.main.async {
                        result(dictionaries)
                    }
                }
    ...
    private func serializeWorkoutsFallback(_ samples: [HKWorkout]) -> [NSDictionary] {
        return samples.map { sample in
            return self.makeWorkoutDictionary(for: sample, planDictionary: nil)
        }
    }
    
    @available(iOS 13.0, *)
    private func serializeWorkouts(_ samples: [HKWorkout]) async -> [NSDictionary] {
        var output: [NSDictionary] = []
        for sample in samples {
            var planDictionary: [String: Any]? = nil
#if canImport(WorkoutKit)
    ...
    private func makeWorkoutDictionary(for sample: HKWorkout, planDictionary: [String: Any]?) async -> NSDictionary {
        var dict: [String: Any?] = [
            ...
            "metadata": sanitizeMetadata(sample.metadata),
            "workout_events": serializeWorkoutEvents(sample.workoutEvents)
        ]
    ...
    private func sanitize(dictionary: [String: Any?]) -> NSDictionary {
        ...
        return cleaned as NSDictionary
    }
    ...
    private func serializeWorkoutEvents(_ events: [HKWorkoutEvent]?) -> [[String: Any]]? {
        ...
        return events.map { event in
            var payload: [String: Any] = [
                "type": workoutEventTypeString(event.type),
                "start": Int(event.dateInterval.start.timeIntervalSince1970 * 1000)
            ]
            if event.dateInterval.duration > 0 {
                payload["end"] = Int(event.dateInterval.end.timeIntervalSince1970 * 1000)
                payload["duration_s"] = event.dateInterval.duration
            }
            if let metadata = event.metadata {
                payload["metadata"] = sanitizeMetadata(metadata)
            }
            return payload
        }
    }
    ...
    @available(iOS 16.0, *)
    private func serializeWorkoutActivities(_ activities: [HKWorkoutActivity]) -> [[String: Any]]? {
        guard !activities.isEmpty else { return nil }
        return activities.map { activity in
            var payload: [String: Any] = [
                "uuid": activity.uuid.uuidString,
                "start": Int(activity.startDate.timeIntervalSince1970 * 1000),
                "duration_s": activity.duration,
                "workoutConfiguration": serializeWorkoutConfiguration(activity.workoutConfiguration),
                "workoutEvents": serializeWorkoutEvents(activity.workoutEvents),
    ...
    private func serializeWorkoutConfiguration(_ configuration: HKWorkoutConfiguration) -> [String: Any] {
        ...
        if let lapLength = configuration.lapLength {
            payload["lapLengthMeters"] = lapLength.doubleValue(for: HKUnit.meter())
        }
        return payload
    }
    
    private func workoutEventTypeString(_ type: HKWorkoutEventType) -> String {
        switch type {
    ...
    private func locationTypeString(_ type: HKWorkoutSessionLocationType) -> String {
        switch type {
...
#if canImport(WorkoutKit)
    @available(iOS 17.0, *)
    private func fetchWorkoutPlanDictionary(for workout: HKWorkout) async -> [String: Any]? {
        do {
            if let plan = try await workout.workoutPlan {
                return serializeWorkoutPlan(plan)
            }
        } catch {
            print("SwiftHealthPlugin: failed to fetch workoutPlan for \(workout.uuid): \(error)")
        }
        return nil
    }
    
    @available(iOS 17.0, *)
    private func serializeWorkoutPlan(_ plan: WorkoutKit.WorkoutPlan) -> [String: Any] {
        return [
            "id": plan.id.uuidString,
            "workout": serializeWorkoutPlanWorkout(plan.workout)
        ]
    }
    ...
    private func serializeWorkoutPlanWorkout(_ workout: WorkoutKit.WorkoutPlan.Workout) -> [String: Any] {
        ...
        case .custom(let customWorkout):
            return [
                ...
                "cooldown": serializeWorkoutStep(customWorkout.cooldown) as Any
            ]
        ...
        case .swimBikeRun(let triWorkout):
            return [
                "type": "swimBikeRun",
                "displayName": triWorkout.displayName as Any,
                "activities": triWorkout.activities.map { activity in
                    ...
                    case .running(let location):
                        return ["segment": "run", "location": locationTypeString(location)]
                    }
                }
            ]
        }
    ...
    @available(iOS 17.0, *)
    private func serializeIntervalBlock(_ block: WorkoutKit.IntervalBlock) -> [String: Any] {
        return [
            "iterations": block.iterations,
            "steps": block.steps.map(serializeIntervalStep)
        ]
    }
    
    @available(iOS 17.0, *)
    private func serializeIntervalStep(_ step: WorkoutKit.IntervalStep) -> [String: Any] {
        return [
            "purpose": step.purpose == .work ? "work" : "recovery",
            "step": serializeWorkoutStep(step.step) as Any
        ]
    }
    
    @available(iOS 17.0, *)
    private func serializeWorkoutStep(_ step: WorkoutKit.WorkoutStep?) -> [String: Any]? {
        guard let step = step else { return nil }
        var payload: [String: Any] = [
            "goal": serializeWorkoutGoal(step.goal)
        ]
        if let alertDict = serializeWorkoutAlert(step.alert) {
            payload["alert"] = alertDict
        }
        if #available(iOS 18.0, *), let displayName = step.displayName {
    ...
    @available(iOS 17.0, *)
    private func serializeWorkoutGoal(_ goal: WorkoutKit.WorkoutGoal) -> [String: Any] {
        switch goal {
        case .open:
            return ["type": "open"]
        case .distance(let value, let unit):
            return ["type": "distance", "value": value, "unit": unit.symbol]
        case .time(let value, let unit):
            return ["type": "time", "value": value, "unit": unit.symbol]
        case .energy(let value, let unit):
            return ["type": "energy", "value": value, "unit": unit.symbol]
        default:
            return ["type": "unknown"]
        }
    }
    ...
    @available(iOS 17.0, *)
    private func serializeWorkoutAlert(_ alert: (any WorkoutKit.WorkoutAlert)?) -> [String: Any]? {
        guard let alert = alert else { return nil }
        
        func metricDictionary(_ metric: WorkoutKit.WorkoutAlertMetric) -> String {
            switch metric {
        ...
        switch alert {
        case let range as WorkoutKit.HeartRateRangeAlert:
            return [
                "type": "heartRateRange",
                "targetLower": measurementDictionary(range.target.lowerBound),
                "targetUpper": measurementDictionary(range.target.upperBound),
                "metric": metricDictionary(range.metric)
            ]
        case let zone as WorkoutKit.HeartRateZoneAlert:
            return [
                "type": "heartRateZone",
                "zone": zone.zone,
                "metric": metricDictionary(zone.metric)
            ]
        case let cadence as WorkoutKit.CadenceThresholdAlert:
            return [
                "type": "cadenceThreshold",
                "target": measurementDictionary(cadence.target),
                "metric": metricDictionary(cadence.metric)
            ]
        case let cadenceRange as WorkoutKit.CadenceRangeAlert:
            return [
                "type": "cadenceRange",
                "targetLower": measurementDictionary(cadenceRange.target.lowerBound),
                "targetUpper": measurementDictionary(cadenceRange.target.upperBound),
                "metric": metricDictionary(cadenceRange.metric)
            ]
        case let power as WorkoutKit.PowerThresholdAlert:
            return [
                "type": "powerThreshold",
                "target": measurementDictionary(power.target),
                "metric": metricDictionary(power.metric)
            ]
        case let powerRange as WorkoutKit.PowerRangeAlert:
            return [
                "type": "powerRange",
                "targetLower": measurementDictionary(powerRange.target.lowerBound),
                "targetUpper": measurementDictionary(powerRange.target.upperBound),
                "metric": metricDictionary(powerRange.metric)
            ]
        case let powerZone as WorkoutKit.PowerZoneAlert:
            return [
                "type": "powerZone",
                "zone": powerZone.zone,
                "metric": metricDictionary(powerZone.metric)
            ]
        case let speed as WorkoutKit.SpeedThresholdAlert:
            return [
                "type": "speedThreshold",
                "target": measurementDictionary(speed.target),
                "metric": metricDictionary(speed.metric)
            ]
        case let speedRange as WorkoutKit.SpeedRangeAlert:
            return [
                "type": "speedRange",
                "targetLower": measurementDictionary(speedRange.target.lowerBound),
                "targetUpper": measurementDictionary(speedRange.target.upperBound),
                "metric": metricDictionary(speedRange.metric)
            ]
        default:
            return ["type": "unknown"]
        }
    ...
    @available(iOS 17.0, *)
    private func measurementDictionary<T: Unit>(_ measurement: Measurement<T>) -> [String: Any] {
        return [
